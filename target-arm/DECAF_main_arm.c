...
int DECAF_is_in_kernel(CPUArchState *env)
{
  return (env->uncached_cpsr & CPSR_M) != ARM_CPU_MODE_USR;
}

gva_t DECAF_getPC(CPUArchState* env)
{
  if (env == NULL)
  {
    return (INV_ADDR);
...
gpa_t DECAF_getPGD(CPUArchState* env)
{
  if (env == NULL)
  {
    return (INV_ADDR);
...
gva_t DECAF_getESP(CPUArchState* env)
{
  if (env == NULL)
  {
    return (INV_ADDR);
...
target_ulong DECAF_getFirstParam(CPUArchState* env)
{
  if (env == NULL)
  {
    //what to do?
...
gva_t DECAF_getReturnAddr(CPUArchState* env)
{
  if (env == NULL)
  {
    return (INV_ADDR);
...
//monitor_printf(default_mon, "virt_addr = %x phy_addr %x", addr, phys_addr);
