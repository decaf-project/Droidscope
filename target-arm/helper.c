 ...
 */
static uintptr_t v2p_mmu__(CPUARMState *env, target_ulong addr, int is_user)
{
    int index;
 ...
 */
uintptr_t v2p(CPUArchState *env, target_ulong ptr, int is_user)
{
    int index;
    target_ulong addr;
    uintptr_t physaddr;
    ...
    if (__builtin_expect(env->tlb_table[is_user][index].addr_read !=
        ...
        physaddr = v2p_mmu__(env, addr, is_user);
    } else {
        physaddr = addr + env->tlb_table[is_user][index].addend;
    }
    return physaddr;
