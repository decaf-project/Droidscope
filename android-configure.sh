...
echo "Configuring Sleuthkit (32-bit) now! Please wait"
cd ./DECAF_shared/sleuthkit
./configure --disable-java --enable-silent-rules --prefix=$PWD \
            CC=gcc-4.8 CXX=g++-4.8 LD=gcc-4.8 \
            CFLAGS=-m32 CXXFLAGS=-m32 LDFLAGS=-m32
cd ../..

echo "Configuring Sleuthkit (64-bit) now! Please wait"
cd ./DECAF_shared/sleuthkit64
./configure --disable-java --enable-silent-rules --prefix=$PWD \
            CC=gcc-4.8 CXX=g++-4.8 LD=gcc-4.8 \
