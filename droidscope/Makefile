...
SLEUTHKIT_LIB=$(SRC_PATH)/DECAF_shared/sleuthkit/lib/libtsk.a
SLEUTHKIT64_LIB=$(SRC_PATH)/DECAF_shared/sleuthkit64/lib/libtsk.a
SLEUTHKIT_SHARED_LIB=$(SRC_PATH)/DECAF_shared/sleuthkit/lib/libtsk.so
...
.NOTPARALLEL: sleuthkit

.PHONY: emualtor libraries executables clean clean-config clean-objs-dir \
...
emualtor: sleuthkit libraries executables
EXECUTABLES :=
LIBRARIES   :=
...
distclean: clean clean-config
	#rm -rf $(SLEUTHKIT_LIB)
	$(MAKE) -C ./DECAF_shared/sleuthkit uninstall > /dev/null 2>&1
	$(MAKE) -C ./DECAF_shared/sleuthkit distclean > /dev/null 2>&1
	#rm -rf $(SLEUTHKIT64_LIB)
	$(MAKE) -C ./DECAF_shared/sleuthkit64 uninstall > /dev/null 2>&1
	$(MAKE) -C ./DECAF_shared/sleuthkit64 distclean > /dev/null 2>&1
...
sleuthkit: $(SLEUTHKIT_SHARED_LIB)

$(SLEUTHKIT_SHARED_LIB):
	@echo "Building Sleuthkit (32-bit), please wait..."
	$(MAKE) -j32 -C ./DECAF_shared/sleuthkit > /dev/null 2>&1
	$(MAKE) -C ./DECAF_shared/sleuthkit install > /dev/null 2>&1
	@echo "Building Sleuthkit (64-bit), please wait..."
