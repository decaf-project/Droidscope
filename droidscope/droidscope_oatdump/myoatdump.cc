...
static std::unordered_map <uint32_t, std::unique_ptr<OatDumper>> oatmodule_dumpers;
...
class OatDumper {
 public:
  ...
  ~OatDumper() {
	...
	//ElfFile *elf = oat_file_.GetElfFile();
	
	const byte * base = oat_file_.Begin();
	size_t size = oat_file_.Size();
	munmap(const_cast<void *>(reinterpret_cast<const void *>(base)), size);
	std::cout << "size = " << size << " base = " << reinterpret_cast<uintptr_t>(base) << std::endl;
  }
...
#undef DUMP_OAT_HEADER_OFFSET
  ...
  const void* GetQuickOatCode(mirror::ArtMethod* m) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    std::unordered_map<uint32_t, std::string> method_offsets;
      ...
      for (size_t class_def_index = 0;
        ...
        if (class_data != nullptr) {
          ...
          while (it.HasNextDirectMethod()) {
              ...
              method_offsets[code_offset] = PrettyMethod(it.GetMemberIndex(), *dex_file, true);
            AddOffsets(oat_method);
            it.Next();
          }
...
static int oatdump(uint64_t base, const char *file_path) {

  if(oatmodule_dumpers.count(base))
  	return 0;
	
  char **argv = nullptr;
  InitLogging(argv);
  
  std::string oat_filename =  file_path;
  std::string elf_filename_prefix;
  std::unique_ptr<std::ofstream> out;
  bool dump_raw_mapping_table = false;
  bool dump_raw_gc_map = false;
  bool dump_vmap = false;
  ...
  if (oat_filename != nullptr) {
    ...
    if (oat_file == nullptr) {
      fprintf(stderr, "Failed to open oat file from '%s': %s\n", oat_filename.c_str(), error_msg.c_str());
      return EXIT_FAILURE;
    }

	std::unique_ptr<OatDumper> oat_dumper_ptr(new OatDumper(*oat_file, oat_dumper_options.release()));
	
    oatmodule_dumpers[base] = std::move(oat_dumper_ptr);
    
	std::cout << "SUCCESSFULLY loaded " << 	oat_filename << "\n";
	bool success = true;
...
// Take a file_path of an OAT file and add its symbols into locat state
int entry_main(uint32_t base, const char *file_path) {
  return art::oatdump( base, file_path);
}
...
int art_method_at(uint32_t base, uint32_t offset, char *method_name) {
    ...
    if(!art::oatmodule_dumpers.count(base))
        return 0;


    art::OatDumper *oat_dumper = art::oatmodule_dumpers[base].get();

    if(!(oat_dumper->method_offsets.count(offset)))
        return 0;

    strcpy(method_name, oat_dumper->method_offsets[offset].c_str());
...
{		
	//art::oatmodule_dumpers.clear();
}


void test()
{
	std::cout << "this is a test from my oatdump\n" ;
}
