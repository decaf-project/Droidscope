...
#undef ART_USE_PORTABLE_COMPILER
#endif
namespace art {
...
class MANAGED ArtMethod FINAL : public Object {
 public:
  // INSERTED FOR DROIDSCOPE! -Abishek
  ...
  static MemberOffset GetDexMethodIndexOffset() {
    return OFFSET_OF_OBJECT_MEMBER(ArtMethod, dex_method_index_);
  }

  static MemberOffset GetDexCodeItemOffset() {
    return OFFSET_OF_OBJECT_MEMBER(ArtMethod, dex_code_item_offset_);
  }


  friend uint32_t GetDexMethodIndexOffset_f(ArtMethod *m) {
    return m->dex_method_index_;
  }
