...
public:
  ...
  : dex_file_(dex_file), pos_(0), ptr_pos_(raw_class_data_item), last_idx_(0), env_(env), cr3_(cr3) {
    ReadClassDataHeader();
...
private:
  ...
  {
    CHECK(ptr_pos_ != NULL);
    header_.static_fields_size_ = MyDecodeUnsignedLeb128(&ptr_pos_);
    header_.instance_fields_size_ = MyDecodeUnsignedLeb128(&ptr_pos_);
    header_.direct_methods_size_ = MyDecodeUnsignedLeb128(&ptr_pos_);
    header_.virtual_methods_size_ = MyDecodeUnsignedLeb128(&ptr_pos_);
  }
  ...
  {
    method_.method_idx_delta_ = MyDecodeUnsignedLeb128(&ptr_pos_);
    method_.access_flags_ = MyDecodeUnsignedLeb128(&ptr_pos_);
    method_.code_off_ = MyDecodeUnsignedLeb128(&ptr_pos_);
...
uint32_t MyDecodeUnsignedLeb128(const uint8_t** data) {
  char read_buffer[10];
  const uint8_t *ptr = *data, *base_ptr;
  const uint8_t *original_ptr = ptr;
  DECAF_read_mem_with_pgd(env_, pgd_strip(cr3_), ptr, read_buffer, 10);
  ptr = reinterpret_cast<uint8_t*>(read_buffer);
  base_ptr = ptr;
  ...
  const uint8_t *original_to_return = original_ptr + (ptr - base_ptr);
  *data = original_to_return;
