...
web site at:
http://www.cs.ucr.edu/~heng/software.html
...
static void extract_art_offsets_from_module(target_ulong base, module* dirty_module, CPUArchState* env, target_ulong cr3, std::string moduleName) {
  ...
  // And for all the classes, we skip the fields, extract method information 
  for (size_t i = 0; i < oat_dex_files.size(); i++) {
    ...
    if (dex_file == nullptr) {
      std::cout << "Failed to open dex file '" << oat_dex_file->GetDexFileLocation() << "': " << error_msg;
      continue;
    }
    else {
...
static void extract_art_offsets__(target_ulong base_, target_ulong size, std::string moduleName, CPUArchState* env, target_ulong cr3, bool forceExtract) {
  ...
  for (target_ulong oat_file_base = base_; oat_file_base != oat_file_end; oat_file_base += 1) {
    ...
    DECAF_read_mem_with_pgd(env, cr3, oat_file_base, (void*)&ph, sizeof(uint8_t));
  ...
  const std::vector<const art::OatFile::OatDexFile*> oat_dex_files_ = oat_file->GetOatDexFiles();
...
static art::DexFile* extractDexFile(CPUArchState* env, target_ulong dex_cache, target_ulong declaring_class, 
  ...
  art::mirror::HeapReference<art::mirror::Class>* objref_addr = reinterpret_cast<art::mirror::HeapReference<art::mirror::Class>*>(raw_addr);
  ...
  art::mirror::HeapReference<art::mirror::DexCache>* dexcache_objref_addr = reinterpret_cast<art::mirror::HeapReference<art::mirror::DexCache>*>(raw_addr);
...
static void module_load_cb(VMI_Callback_Params* params) {
  ...
  // add more packer signatures here
...
static void block_begin_cb(DECAF_Callback_Params* param)
{
  ...
  */
  ...
  // dump the functions called in libc and libart module
  if(art_module != NULL && (strstr(art_module->name, "libc") || strstr(art_module->name, "libart"))) {
  ...
  if(art_module != NULL && strstr(art_module->name, "libart")) {
    if(funcmap_get_name_c(cur_pc, DECAF_getPGD(env), modname, functionname) == 0) {
      ...
      if (reg_num != NON_INVOKE_CALL) {
        ...
        /**********************************************************************/
        /* WE GOT THE DEXFILE! NOW TIME TO GET THE FUNCTION NAME */
        /**********************************************************************/
        ...
        */
        ...
        if(reg_num == ARTMETHOD_INVOKE) {
          ...
          DECAF_read_mem_with_pgd(env, pgd_strip(cr3), dex_begin + (target_ulong)(*code_item_offset), code_item_block, sizeof(art::DexFile::CodeItem));
