...
#define mysprintf(arg, ...) if(arg != NULL) 			     \
								sprintf(arg + strlen(arg), ##__VA_ARGS__); \


static const char* gProgName = "dexdump";
static unordered_map<uint32_t, struct DexFile *> dex_file_map;

bool isExtractRequired(uint32_t base)
{
    if(dex_file_map.count(base))
        return false;

    return true;
}

bool parseDexAndAdd(uint32_t base, const char *file_name)
{
    DexFile* pDexFile = NULL;
    MemMapping map;
    
    if(dexOpenAndMap(file_name, NULL , &map, false) != kUTFRSuccess)
		return false;
...
*/
void dumpInstruction(DexFile* pDexFile, const u2* insns, int insnIdx,
int insnWidth, const DecodedInstruction* pDecInsn, char *output)
{
    char indexBufChars[400];
...
void dumpOneInstruction(uint32_t base,const u2 * insns, char *output)
{
	DexFile *pDexFile = dex_file_map[base];
    int insnIdx = 0;
    int insnWidth;
    DecodedInstruction decInsn;
    u2 instr;
    ...
    dexDecodeInstruction(insns, &decInsn);
	dumpInstruction(pDexFile, insns, insnIdx, insnWidth,&decInsn, output);
}
