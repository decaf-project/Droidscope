...
void helper_DECAF_invoke_block_begin_callback(CPUArchState* env, TranslationBlock* tb, gva_t cur_pc)
{
  callback_struct_t *cb_struct;
...
void helper_DECAF_invoke_block_end_callback(CPUArchState* env, TranslationBlock* tb, gva_t from, gva_t to)
{
  callback_struct_t *cb_struct;
  DECAF_Callback_Params params;
...
void helper_DECAF_invoke_syscall_callback(CPUArchState* env, gva_t pc, target_ulong num)
{
...
void DECAF_invoke_PGD_write_callback(CPUArchState* env, gpa_t oldpgd, gpa_t newpgd
#ifdef TARGET_ARM
, uint32_t c2base
#endif
)
